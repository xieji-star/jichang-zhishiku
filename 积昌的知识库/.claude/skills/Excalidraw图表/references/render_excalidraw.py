...
Usage:
    cd Skills/excalidraw-diagram-skill/references
...
def compute_bounding_box(elements: list[dict]) -> tuple[float, float, float, float]:
    ...
    for el in elements:
        # Caller already filters deleted elements before passing to this function
        x = el.get("x", 0)
        y = el.get("y", 0)
        w = el.get("width", 0)
        h = el.get("height", 0)
...
def render(
    ...
    try:
        ...
        print("Run: cd .claude/skills/Excalidraw图表/references && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd .claude/skills/Excalidraw图表/references && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
        ...
        try:
            ...
            # Use 120s timeout since esm.sh can be slow to bundle @excalidraw/excalidraw
            page.wait_for_function("window.__moduleReady === true", timeout=120000)

            # If module failed to load, report that error
            module_error = page.evaluate("window.__moduleError")
            if module_error:
                print(f"ERROR: Excalidraw module failed to load: {module_error}", file=sys.stderr)
                sys.exit(1)

            # Inject the diagram data and render
            # (page.evaluate awaits async functions, so __renderComplete is guaranteed true on return)
